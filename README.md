...
flowchart TD
    ...
    B -->|조회| C[Redis Cache<br/>ElastiCache]
...
- 🔐 모든 시크릿(DB 비밀번호, API 키)은 **AWS Secrets Manager**로 관리
...
### 7️⃣ Celery 워커 실행 (AI 리포트 기능 필요 시)
...
Redis (AWS ElastiCache, `eu-west-3`)를 **Cache-Aside 패턴**으로 사용합니다.
...
> 📄 전체 스키마(13개 테이블, 55개 인덱스)는 [Wiki: DB 스키마 v5.0](./DB-스키마.md) 참고
...
| 문서 | 내용 |
|---|---|
| [API 명세서 v5.0](./API-명세서.md) | 전체 14개 엔드포인트 요청/응답 스펙, v5.0 변경이력 |
| [DB 스키마 v5.0](./DB-스키마.md) | 13개 테이블 컬럼 정의, 인덱스, 마이그레이션 이력 |
