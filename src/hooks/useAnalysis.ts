...
import { getIssues, getPrice, getPriceStats, getPriceTrend } from '../api/analysis';

// 가격 현황 조회 훅
export function usePrice(regionId: string, lawdCd: string, dealYmd: string) {
  return useQuery({
    queryKey: ['price', regionId, lawdCd, dealYmd],
    queryFn: () => getPrice(regionId, lawdCd, dealYmd),
